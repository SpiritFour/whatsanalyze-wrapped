...
const locales: Array<{ code: LocaleCode; name: string; flag: string; iso: string }> = [
...
<template>
  <div class="relative inline-block">
    <label class="sr-only" for="lang-switch">{{ $t('nav.selectLanguage') }}</label>
    <select
        id="lang-switch"
        v-model="selected"
        @change="onChange"
        class="appearance-none pr-10 pl-3 py-2 rounded-xl bg-zinc-900/80 text-zinc-200 ring-1 ring-zinc-800/80
             hover:ring-zinc-700 focus:outline-none focus:ring-2 focus:ring-emerald-500
             shadow-inner backdrop-blur transition-colors text-sm"
